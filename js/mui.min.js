	...
	plus.oauth.getServices(function (services) {
		...
		wxLoginObj.login(function (res) {
			wxLoginObj.getUserInfo(function(){
				var unionid = wxLoginObj.authResult.unionid;
				var nickname = wxLoginObj.userInfo.nickname;
				var face = wxLoginObj.userInfo.headimgurl;
				mui.post(
					'http://192.168.31.242/ksbapi2/api/user/loginbyweixin',
					{
						nickname: nickname,
						face: face,
						unionid: unionid,
					...
					function (data) {
						if(data.data.user.userstatus == -1){
							mui.toast('请联系系统管理员激活您的用户信息');
						} else {
							plus.storage.setItem('uid',data.data.user.id+'');
							plus.storage.setItem('token',data.data.token);
							plus.storage.setItem('face',data.data.user.photourl);
							plus.storage.setItem('name',data.data.user.name);
							mui.toast('登录成功');
							eval(goto);
						}
					},
...
function logoff() {
	plus.oauth.getServices(function (services) {
		...
		wxLoginObj.logout(function() {
			...
			plus.storage.removeItem('face');
			plus.storage.removeItem('name');
			mui.toast('注销成功');
			_index.evalJS("var nav1 = document.getElementById('nav1'); mui.trigger(nav1, 'tap'); h('#tabBar').find('a').removeClass('mui-active'); h('#nav1').addClass('mui-active');");
		},function(){
